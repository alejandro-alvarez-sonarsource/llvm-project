...
void MallocChecker::preGetdelim(const CallEvent &Call,
  ...
  ProgramStateRef State = C.getState();
  auto const *LineArgExpr = Call.getArgExpr(0);

  auto LinePtr = getPointeeDefVal(Call.getArgSVal(0), State);
  if (!LinePtr)
    return;

  bool IsKnownToBeAllocated = false;
  State = FreeMemAux(C, LineArgExpr, Call, State, false, IsKnownToBeAllocated,
                     AF_Malloc, false, LinePtr);
  if (State)
    C.addTransition(State);
}
...
void MallocChecker::checkGetdelim(const CallEvent &Call,
  ...
  auto LinePtr = getPointeeDefVal(Call.getArgSVal(0), State);
  auto Size = getPointeeDefVal(Call.getArgSVal(1), State);
  if (!LinePtr || !Size)
    return;
